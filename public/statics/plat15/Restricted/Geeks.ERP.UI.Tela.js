(function () {
  Geeks.ERP.UI.Tela = new function () {
    var uiTela = this;
    var editingSource = false;

    var dataSource = function (objeto) {

      var _fillSessaoCampo = function (sessao, result) {
        ...
        $.each(camposTabelaMasterObjeto, function (index, campo) {
          var value = result && result.length > 0 ? result[0][campo.Id] : null;

          if (campo.TipoCampo == "data") {
            if (value != null) {
              var dateMillis = new Date(parseInt(value.substr(6))).getTime();// + (new Date().getTimezoneOffset() * 60000) - (300 * 60000);
          ...
          var campoPrompt = $.grep(objeto.Campos, function (value) {
          ...
          if (campo.TipoCampo == "numero" || campo.TipoCampo == "valor" || campo.TipoCampo == "valor4" || campo.TipoCampo == "moeda") {
            var input = objeto.Target.find("[name=" + campo.Id + "]");
            var decimal = 0;
      ...
      var _insert = function (sessoes, content) {
        var erros = "";
      ...
      return {
        ...
        Fill: function (filter) {
          ...
          var result = Geeks.ERP.Core.Connection.ExecuteSQL(sql, objeto.Estrutural);
          ...
          if (campoIdentificador.length > 0) {
            ...
            result = Geeks.ERP.Core.Connection.ExecuteSQL(sql, objeto.Estrutural);
        ...
        AddNew: function () {
          ...
          var includeBody = uiTela.OpenWindow("popupAdd", objeto.Target, obj, null, true, true, false);
          ...
          includeBody.find("a[name=btn_Salvar]").click(function () {
    ...
    this.OpenWindow = function (windowType, container, objeto, callback, renderObjeto, codeNotEditable, reloadCodeOnLoad) {
      ...
      if (windowType != "gridEdit" && windowType != "popup" && windowType != "popupAdd" && windowType != "popupEdit" && objeto.CodigoObjeto != 0) {
        ...
        var sql =
          ...
          "else " +
          "	Select 0 Permitir ";
        var retorno = Geeks.ERP.Core.Connection.ExecuteSQL(sql, true).Records;
        ...
        //Grava no LogMenu
        var sql = "Insert into LogMenu (CodigoUsuario, CodigoContato, Nome, CodigoMenu, DescricaoMenu, DataHoraAcesso) " +
      ...
      if (reloadCodeOnLoad) {
        try {
          ...
          objeto.CodigoPersonalizado = Geeks.ERP.Core.Connection.ExecuteSQL("select CodigoPersonalizado From Objeto Where CodigoObjeto = " + objeto.CodigoObjeto, true).Records[0].CodigoPersonalizado
        } catch (ex) {
          //Nothing to do.
        }
      }
      ...
      if (contentBody.length <= 0) {
        ...
        ulbotoes.
          append(uiTela.MontaBotoes(botoesFixos, null));
          ...
          if (windowType == "content") {
            ...
            if (openedWindows) {
              ...
              if (previousWindow && $("#" + previousWindow.attr("id")).length > 0) {
                ...
                previousWindow.show();
                uiTela.integracao(previousWindow.attr("id"), previousWindow);
        ...
        //#endregion
        if (renderObjeto)
          uiTela.RenderObjeto(contentBody, objeto, windowType == "gridEdit" || windowType == "popupEdit" || windowType == "popupAdd");

        uiTela.FormataCampos(contentBody);
    ...
    this.RenderObjeto = function (contentBody, objeto, utilizaPropriedadesObjeto) {
      var uiGrid = Geeks.ERP.UI.Grid;
      ...
      contentBody.find("ul[name='MenuBotoes']")
        .prepend(uiTela.MontaBotoes(objeto.Botoes, null));

      var larguraSessao = 0;

      $.each(objeto.Sessoes, function (key, sessao) {

        var sessaoBody = contentBody.find(".fluid");
          ...
          if (sessao.TipoSessao != "Campo") {
            if (sessao.Tamanho == "" || sessao.Tamanho == null) {
              if (sessaoBody.parents(".wrapper").parent().attr("ref") == "contentBody" && $(window).height() > 300) {
                var height = $(window).height() - sessaoBody.parents(".wrapper").parent().position().top;
        ...
        if (sessao.TipoSessao.toLowerCase() == "grade") {
          ...
          if (grid.length > 0) {
            ...
            sessaoBody.find("a[name=PropriedadeGrid]").click(function () {
              var propriedadeGrid = uiTela.OpenWindow("popup", contentBody, { CodigoObjeto: 0, NomeObjeto: "Propriedades", Descricao: "Propriedades da Grade \"" + sessao.Descricao + "\"", Estrutural: true }, null, false, true);
          ...
          $.each(campos, function (index, campo) {
            ...
            if (campoEForeignKey.length <= 0) {
              ...
              formRow.append(uiTela.MontaCampos(sessao.Ordem, campo, objeto.Record, false, objeto.Estrutural));
        ...
        sessaoBody.find("a[name=BotaoSessao]").click(function () {
          var campos = [];

          var opcoes = JSON.parse(Geeks.ERP.Core.Connection.ExecuteSQL("Select g.* from Sessao s left join Grid as g on g.CodigoSessao = s.CodigoSessao where s.CodigoSessao = 12 ", true).Records[0].DtaLayout);
          ...
          var propriedadeSessao = uiTela.OpenWindow("popupEdit", contentBody, obj, null, true, true);
          ...
          propriedadeSessao.find("a[name=btn_Salvar]").click(function () {
            var campo = "";
            var where = "";
            ...
            Geeks.ERP.Core.Connection.ExecuteSQL('UPDATE Sessao SET ' + campo + where, true);
        ...
        sessaoBody.find("a[name=BotaoComponente]").click(function () {
          var idCampos = "";
          var nomeSessao = "";
          ...
          var camposSessao = uiTela.OpenWindow("popup", contentBody, { CodigoObjeto: 0, NomeObjeto: "Sessao", Descricao: nomeSessao + " da Sessão \"" + sessao.Descricao + "\"", Estrutural: true }, null, false, true);
          ...
          var grid = Geeks.ERP.Core.Connection.ExecuteSQL("select * from Grid Where CodigoGrid = " + idCampos, true).Records[0];
      ...
      objeto.DataSource = dataSource(objeto);
    ...
    this.MontaBotoes = function (botoes, codigoPai) {
      var ret = $("<div>");
      ...
      $.each(nivelBotao, function (key, botao) {
        ...
        if (botoesFilhos.length > 0) {
          ...
          ul.append(uiTela.MontaBotoes(botoesFilhos, botao.CodigoBotao));
        }
    ...
    this.MontaCampos = function (tabindex, campo, record, force100Percent, structural) {
      var retorno = "";
    ...
    this.FormataCampos = function (target) {
    ...
    this.integracao = function (tela, container) {
      if (container[0].id.indexOf("popup") < 0) {
        if (tela == "PadraoNovo_Contatos") {
          ...
          if (codigoContato != null && codigoContato != "null") {
            ...
            var retorno = Geeks.ERP.Core.Connection.ExecuteSQL(sql);
            if (retorno != null && retorno.Records.length > 0) {
              ...
              retorno = Geeks.ERP.Core.Connection.ExecuteSQL(sql);
          ...
          if (origem == "true")
            ...
            if (codigoContato != null && codigoContato != "null") {
              var nomeContato = localStorage.getItem("nomeContato" + $("#hfCodigoSistema").data("GeeksData").CodigoClienteSistema);
              var retorno = Geeks.ERP.Core.Connection.ExecuteSQL(" Select top 1 CodigoDocumento from Documento where Tipo='Compra' and DataHoraEmissao < getDate() and CodigoContato = 0" + codigoContato + "order by CodigoDocumento desc");
              if (retorno != null && retorno.Records.length > 0) {
                if (container.find("[name=CodigoDocumento]") && container.find("[name=CodigoDocumento]").val() == "") {
                  $.jGrowl("Pedido " + retorno.Records[0].CodigoDocumento + " selecionado.");
          ...
          if (codigoItem != null && codigoItem != "null" && codigoItem != "undefined" && container.find("input[name=Produto]") && container.find("input[name=Produto]").val() == "") {
            ...
            var dados = Geeks.ERP.Core.Connection.ExecuteSQL(sentencaSql);
